...
| `v4.2.0-rc2`, ReSukiSU | [`patches/ReSukiSU-v4.2.0-rc2-samsung-kdp-rkp-defex.patch`](patches/ReSukiSU-v4.2.0-rc2-samsung-kdp-rkp-defex.patch) | The third flavour: ReSukiSU is a SukiSU-Ultra fork on KernelSU's lineage. Its own tracepoint mode writes `sys_call_table` through `ksu_patch_text`, which Samsung's RKP refuses, so the same guard the other two carry is applied here. **Built by CI for `pa3q`; not device-confirmed.** See [`../docs/RESUKISU-4.2.0-RC2-PORT.md`](../docs/RESUKISU-4.2.0-RC2-PORT.md) — what was ported, what this flavour does not have (no `avc_spoof`), and why its version is `30700 + commits` rather than `30000 + commits`. |
