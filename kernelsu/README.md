...
| `v3.3.0` (`3b18216f`), KernelSU-Next | [`patches/KernelSU-Next-v3.3.0-samsung-kdp-rkp-defex.patch`](patches/KernelSU-Next-v3.3.0-samsung-kdp-rkp-defex.patch) | Rebased and built for one target: `android15-6.6_kernelsu-next-pa3q-S938USQSCCZF9-kdp.ko` and `ksud-next-pa3q-S938USQSCCZF9-kdp`, the pair the feed serves as `pa3q-S938USQSCCZF9-ksun330`. **Not device-confirmed.** |
| `v3.4.0` (`1a879d6a`), KernelSU-Next | [`patches/KernelSU-Next-v3.4.0-samsung-kdp-rkp-defex.patch`](patches/KernelSU-Next-v3.4.0-samsung-kdp-rkp-defex.patch) | Rebased and verified against the tag; **not compiled, nothing published.** See [`../docs/KERNELSU-NEXT-3.4.0-UPGRADE.md`](../docs/KERNELSU-NEXT-3.4.0-UPGRADE.md) — the three conflicts and their resolutions, the version that is derived from the tag rather than set by hand, and the readings a device run has to produce. |
| `v4.2.0-rc2`, ReSukiSU | [`patches/ReSukiSU-v4.2.0-rc2-samsung-kdp-rkp-defex.patch`](patches/ReSukiSU-v4.2.0-rc2-samsung-kdp-rkp-defex.patch) | The third flavour: ReSukiSU is a SukiSU-Ultra fork on KernelSU's lineage. Its own tracepoint mode writes `sys_call_table` through `ksu_patch_text`, which Samsung's RKP refuses, so the same guard the other two carry is applied here. Built for one target: `android15-6.6_kernelsu-rsksu-pa3q-S938USQSCCZF9-kdp.ko` and `ksud-rsksu-pa3q-S938USQSCCZF9-kdp`, the pair the feed serves as `pa3q-S938USQSCCZF9-rsksu420`. **Not device-confirmed.** See [`../docs/RESUKISU-4.2.0-RC2-PORT.md`](../docs/RESUKISU-4.2.0-RC2-PORT.md) — what was ported, what the first compile found, what this flavour does not have (no `avc_spoof`), and why its version is `30700 + commits` rather than `30000 + commits`. |
| `v4.2.0-rc3` (`239e1e88`), ReSukiSU | [`patches/ReSukiSU-v4.2.0-rc3-samsung-kdp-rkp-defex.patch`](patches/ReSukiSU-v4.2.0-rc3-samsung-kdp-rkp-defex.patch) | The rc2 delta carried forward onto rc3; the kernel half is byte-identical and only the two userspace files disagree with upstream's rework of the staged-daemon split. **Rebased and verified against the tag; nothing compiled, nothing published for it yet.** See [`../docs/RESUKISU-4.2.0-RC3-PORT.md`](../docs/RESUKISU-4.2.0-RC3-PORT.md). |
...
Three flavours are carried - KernelSU, KernelSU-Next and ReSukiSU - and a pair names which one it
belongs to: `ksud-<target>-kdp` is KernelSU, `-next` is KernelSU-Next and `-rsksu` is ReSukiSU. The
kernel half is named `<kmi>_kernelsu<suffix>-<target>-kdp.ko` in every flavour, because all three
load it from their daemon's own asset directory as `<kmi>_kernelsu.ko`; the suffix distinguishes
the published files, not the embedded one. Their version arithmetic differs - KernelSU and
KernelSU-Next count `30000 + commits`, ReSukiSU `30700 + commits` - which is why every check that
reads a version takes the flavour as an input.
